...
const Sections = () => {
  ...
  const [GetListDataslist, setGetListDataslist] = useState();
  const [GetListDatas, setGetListDatas] = useState();
  ...
  const GetListData = async (search) => {
    ...
    try {
      ...
      setGetListDataslist(Response.data.data);
      setGetListDatas(Response.data);
      setloading(false);
    } catch (error) {
  ...
  useEffect(() => {
    if (SectionsDataReload === true) {
      GetListData();
      setSectionsDataReload(false);
    }
  }, [SectionsDataReload]);
  useEffect(() => {
    if (searchTerm.length > 2 || searchTerm.length === 0) {
      const delayDebounceFn = setTimeout(() => {
        GetListData(searchTerm);
      });
  ...
  const handlePageClick = (pageNumber) => {
    if (pageNumber >= 1 && pageNumber <= GetListDatas?.pagedata?.totalpage) {
      setActivePage(pageNumber);
    }
  };

  const renderPaginationItems = () => {
    const totalPages = GetListDatas?.pagedata?.totalpage || 1;
  ...
  const handleEditModel = (id) => {
    setSectionsDataEditModel(true);
    setSelectedSectionsData(id);
  };
  const handleDeleteModel = (id) => {
    setSectionsDataDeleteModel(true);
    setSelectedSectionsData(id);
  };
        ...
        <div className="All-Containor-perfect-second-divv">
          <div className="heding_div">
            <div className="addbtn_div">
              <NavLink
                onClick={setSectionsDataAddModel}
            ...
            <MDBTable align="middle">
              ...
              <MDBTableBody className="table_body">
                {GetListDataslist?.length === 0 ? (
                  ...
                  GetListDataslist?.map((item, index) => {
                    return (
                      <tr>
                        <td>{index + 1}</td>
                        <td style={{ width: "40%" }}>{item.section_name}</td>
                        <td
                          style={{
                            width: "40%",
                            cursor: "pointer",
                            color: "var(--primary-color-lightgreen)",
                            fontWeight: "bold",
                            textAlign: "center",
                            fontSize: "20px",
                          }}
                          onClick={() =>
                            navigate("/Home/Sections-data-by-id", {
                              state: item,
                            })
                          }
                        >
                          {item.total_section_data}
                        </td>
                        <td style={{ width: "20%" }}>
                          <MDBBtn
                            color="dark"
          ...
          <div className="pagination_div">
            <div className="totoalpage_show_div">
              <p>Total Pages: {GetListDatas?.pagedata?.totalpage}</p>
            </div>
            <nav aria-label="Page navigation example">
              <MDBPagination className="mb-0">
                ...
                {/* Next Button */}
                <MDBPaginationItem
                  disabled={activePage === GetListDatas?.pagedata?.totalpage}
                >
                  <MDBPaginationLink
                    onClick={() => handlePageClick(activePage + 1)}
                  >
