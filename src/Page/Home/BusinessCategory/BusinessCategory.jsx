...
const BusinessCategory = () => {
  ...
  const {
    ...
    FaqsEditModel,
    setFaqsEditModel,
    SelectedbusinessCategor,
    setSelectedbusinessCategor,
    FaqsReload,
    setFaqsReload,
  ...
  const [businessCategorslist, setbusinessCategorslist] = useState();
  const [businessCategors, setbusinessCategors] = useState();
  console.log("🚀 ~ Faqs ~ businessCategors:", businessCategors);
  ...
  const businessCategor = async (search) => {
    setloading(true);
    try {
      const Response = await axios.post(
        "master/get-business-category",
        {
          page: activePage,
          keyword: search,
        },
      ...
      setbusinessCategorslist(Response.data.data);
      setbusinessCategors(Response.data);
      setloading(false);
    } catch (error) {
  ...
  useEffect(() => {
    if (BusinessCategoryReload === true) {
      businessCategor();
      setBusinessCategoryReload(false);
    }
  }, [BusinessCategoryReload]);
  ...
  const handlePageClick = (pageNumber) => {
    if (
      pageNumber >= 1 &&
      pageNumber <= businessCategors?.pagedata?.totalpage
    ) {
  ...
  const renderPaginationItems = () => {
    const totalPages = businessCategors?.pagedata?.totalpage || 1;
  ...
  const handleEditModel = (id) => {
    setBusinessCategoryEditModel(true);
    setSelectedBusinessCategoryData(id);
  };
  const handleDeleteModel = (id) => {
    setBusinessCategoryDeleteModel(true);
    setSelectedBusinessCategoryData(id);
  };
  return (
    <>
        ...
        <div className="All-Containor-perfect-second-divv">
          <div className="heding_div">
            <div className="addbtn_div">
              <NavLink
                onClick={setBusinessCategoryAddModel}
            ...
            <MDBTable align="middle">
              ...
              <MDBTableBody className="table_body">
                {businessCategorslist?.length === 0 ? (
                  ...
                  businessCategorslist?.map((item, index) => {
                    return (
                      <tr>
                        <td>{index + 1}</td>
          ...
          <div className="pagination_div">
            <div className="totoalpage_show_div">
              <p>Total Pages: {businessCategors?.pagedata?.totalpage}</p>
            </div>
            <nav aria-label="Page navigation example">
              <MDBPagination className="mb-0">
                ...
                <MDBPaginationItem
                  disabled={
                    activePage === businessCategors?.pagedata?.totalpage
                  }
                >
                  <MDBPaginationLink
                    onClick={() => handlePageClick(activePage + 1)}
