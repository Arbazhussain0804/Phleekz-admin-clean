...
const Supports = () => {
  ...
  const [FaqsDatas, setFaqsDatas] = useState();
  ...
  const FaqsData = async (search) => {
    setloading(true);
    try {
      const Response = await axios.post(
        "support/get-contactus",
        {
          page: activePage,
          keyword: search,
        },
      ...
      setFaqsDataslist(Response.data.data);
      setFaqsDatas(Response.data);
      setloading(false);
    } catch (error) {
  ...
  useEffect(() => {
    if (SupportsReload === true) {
      FaqsData();

      setSupportsReload(false);
    }
  }, [SupportsReload]);
  useEffect(() => {
  ...
  const handlePageClick = (pageNumber) => {
    if (pageNumber >= 1 && pageNumber <= FaqsDatas?.pagedata?.totalpage) {
      setActivePage(pageNumber);
    }
  };

  const renderPaginationItems = () => {
    const totalPages = FaqsDatas?.pagedata?.totalpage || 1;
  ...
  const handleViewModel = (item) => {
    setSelectedSupportsdata(item);
    setSupportsViewModel(true);
  };
  const handleCheckModel = (item) => {
    setSelectedSupportsdata(item);
    setSupportsCheckModel(true);
  };
        ...
        <div className="All-Containor-perfect-second-divv">
                    ...
                    return (
                      <tr>
                        ...
                        <td style={{ width: "20%" }}>
                          ...
                          <MDBBtn
                            disabled={item.is_read === "Yes"}
                            style={{
                              background: "var(--primary-color-lightgreen)",
                            }}
                            rounded
                            size="sm"
                            onClick={() => handleCheckModel(item)}
                          >
                            <FaCheck />
                          </MDBBtn>
                        </td>
          ...
          <div className="pagination_div">
            <div className="totoalpage_show_div">
              <p>Total Pages: {FaqsDatas?.pagedata?.totalpage}</p>
            </div>
            <nav aria-label="Page navigation example">
              <MDBPagination className="mb-0">
                ...
                {/* Next Button */}
                <MDBPaginationItem
                  disabled={activePage === FaqsDatas?.pagedata?.totalpage}
                >
                  <MDBPaginationLink
                    onClick={() => handlePageClick(activePage + 1)}
                  >
